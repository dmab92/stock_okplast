...
class WizardPrintReport(models.TransientModel):
    ...
    tech_ids = fields.Many2many('hr.employee',string="Technicians")
    user_ids = fields.Many2many('res.users', string="Warehouse Person")
    filter_type = fields.Selection([
        ('partner', 'Machine'),
        ('warehouse', 'Usine'),
        ('product', 'Product'),
        ('tech', 'Technicians'),
        ('user', 'Warehouse Person')
    ...
    def export_xlsx_report(self):
        ...
        elif self.filter_type == 'tech' and self.tech_ids:
            domain.append(('tech_id', 'in', self.tech_ids.ids))
        elif self.filter_type == 'user' and self.user_ids:
        ...
        elif self.filter_type == 'tech':
            moves = moves.sorted(lambda m: m.picking_id.tech_id.name or "")
        elif self.filter_type == 'user':
            moves = moves.sorted(lambda m: m.picking_id.user_id.name or "")
        ...
        for move in moves:
            ...
            group_name = (
                ...
                else move.picking_id.tech_id.name
                if self.filter_type == 'tech'
                else move.picking_id.user_id.name
